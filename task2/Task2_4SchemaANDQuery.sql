...
CREATE OR REPLACE FUNCTION count_instances(
    p_period    CHAR(10),
    p_person_id INT
)
RETURNS INT AS $$
...
BEGIN
    ...
    JOIN course_instance ci
      ON ci.id = a.course_instance_id
    WHERE a.person_id   = p_person_id  
      AND ci.study_period = p_period;   
...
SELECT
	e.employee_id AS "Employement ID",
	p.first_name AS "Teacher's first name",
	ci.study_period AS "Period",
	count_instances(ci.study_period, p.id) as "No of courses"
